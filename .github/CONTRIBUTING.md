# Contributing to `jul6art/admin-bundle`

`jul6art/admin-bundle` is the shell of a back office: a sidebar layout, a theme, per-user
appearance, the sign-in pages, and a navigation contract the application fills. Everything
that makes a back office look like a back office, and nothing that makes it yours.
...
* **Bug** → open an [issue](https://github.com/jul6art/admin-bundle/issues/new/choose)
...
```bash
git clone https://github.com/jul6art/admin-bundle.git
cd admin-bundle
composer install
```
...
`jul6art/admin-bundle` is installed by other projects through Composer, so the version
...
3. **The navigation contract stays the seam.** The application declares its items; the
   bundle draws them and asks `security.authorization_checker` whether each one may be
   shown. Never hardcode an application's route or label here.

4. **An item the user may not reach is not drawn.** A visible link to a forbidden page is
   both a bug and an information leak — it tells an attacker the feature exists. The check
   that hides it is not a nicety, and it is not a substitute for the decision on the route
   itself: both are required.

5. **Appearance is per user, and a user only ever writes their own.** Light / dark / system,
   accent, density, text size, contrast, motion — the appearance screen touches the current
   account and nothing else.

6. **A page never flashes the wrong theme.** Appearance is rendered server-side onto
   `<html>`; a change that moves that decision into client-side JavaScript will be refused.
